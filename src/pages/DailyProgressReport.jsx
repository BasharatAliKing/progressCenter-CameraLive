...
function DailyProgressReport() {
  ...
  return (
    <div className="p-4">
      ...
      <div
        ref={dashboardRef}
        className="print-content bg-white rounded-lg shadow-lg p-6"
      >
        <div className="mb-4">
          <ProjectHeader
        ...
        <div className="space-y-4">
          ...
          <ProgressSCurve
            data={
              report?.progressSCurve &&
              report?.progressSCurve.map((s) => ({
                month: s.month,
          ...
          <ProjectProgressTable
            overallProgress={{
              planned: parseFloat(
                report?.overall_schedule_performance_percentage,
              ),
              actual: parseFloat(report?.overall_actual_performance_percentage),
              difference:
                parseFloat(report?.overall_actual_performance_percentage) -
                parseFloat(report?.overall_schedule_performance_percentage),
            }}
            workBreakdownStructure={
              report?.overallProgress &&
              report?.overallProgress.map((w) => ({
                name: w.progress_name,
                planned: parseFloat(w.progress_planned_thisWeek),
                actual: parseFloat(w.progress_actual_thisWeek),
                difference:
                  parseFloat(w.progress_actual_thisWeek) -
                  parseFloat(w.progress_planned_thisWeek),
              }))
            }
          />
          {report?.engineeringQuantity && (
            <EngineeringQualityKPIs data={report?.engineeringQuantity} />
          )}
          {report?.overallProgress && <OverallProgress data={report} />}
