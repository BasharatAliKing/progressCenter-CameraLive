...
function LiveView({ cameraId }) {
  ...
  useEffect(() => {
    const loadStream = async () => {
      try {
        // ✅ Use port 5000
        const res = await fetch(`http://localhost:5000/api/camera/${cameraId}/live`);
        const data = await res.json();
        
        if (!data.success) {
          console.error('Failed to get stream:', data.message);
          return;
        }

        console.log('HLS URL:', data.hlsUrl); // Debug
