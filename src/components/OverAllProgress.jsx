...
export default function OverAllProgress() {
  ...
  const fetchDPRData = async () => {
    try {
      ...
      if (data?.data?.length > 0) {
        setDprData(data.data[0]);
      }
    } catch (err) {
      console.error("Error fetching DPR data:", err);
    }
  };
  // Step 1: filter valid actual values
  const validData = dprData?.progressSCurve.filter(
    (item) => item.planned && item.actual !== "",
  );
  // Step 2: get last item safely
  const lastItem =
    validData?.length > 0 ? validData[validData.length - 1] : null;

  // Step 3: convert % → number
  const actual = parseFloat(lastItem?.actual);
  const planned = parseFloat(lastItem?.planned);
  // Step 4: your logic
  // const actualValue = Math.min(actual, planned);
  const remaining = parseFloat((planned - actual).toFixed(2) || 0);
  const datachart = [
  ...
  const sCurveData = useMemo(() => {
    if (!dprData?.progressSCurve) return [];

    return dprData.progressSCurve.map((item) => ({
      month: item.month,
      planned: parseFloat(item.planned) || 0,
      actual: item.actual ? parseFloat(item.actual) : null, // null = no line
    }));
  }, [dprData]);
  ...
  const last5HoursData = useMemo(() => {
    ...
    const points = Array.from(byHour.entries())
      ...
      .map(([key, r]) => {
        ...
        const fmt = ts.toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        });
        ...
        Object.keys(AQI_PARAM_META).forEach((k) => {
          const v = aq[k];
          row[k] =
            typeof v === "number" ? v : Number.isFinite(v) ? Number(v) : null;
        });
        return row;
      });
  ...
  const handleExportAQI = async () => {
    ...
    try {
      ...
      const hint =
        msg.includes("Failed to fetch") || e?.name === "TypeError"
      ...
      <div className="grid gap-5 grid-cols-12">
          ...
          <div className="bg-white rounded-xl shadow-md p-4">
            ...
            <div className="flex flex-wrap justify-between items-center text-sm font-semibold mt-4 px-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span>
                  Actual Progress:{" "}
                  <span className="text-green-600">{actual.toFixed(2)}%</span>
                </span>
              </div>
        ...
        <div className="bg-gray-100 mb-auto rounded-xl shadow-lg p-6 col-span-4  relative">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Hardware Sensor ({OneCamera?.location}): Live feed and project
            progress
          </p>
              ...
              {last5HoursData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip
                      formatter={(val, name) => {
                        const meta = AQI_PARAM_META[name] || {
                          label: name,
                          unit: "",
                        };
                        return [
